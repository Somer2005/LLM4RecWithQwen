...
class TwoTowerRecaller:
    """基于双塔模型的向量召回"""
    def __init__(self, user_emb_path, user_ids_path, movie_emb_path, movie_ids_path):
        self.user_emb = np.load(user_emb_path)         # shape: (num_users, emb_dim)
        self.user_ids = np.load(user_ids_path)         # shape: (num_users,)
        self.movie_emb = np.load(movie_emb_path)       # shape: (num_movies, emb_dim)
        self.movie_ids = np.load(movie_ids_path)       # shape: (num_movies,)
        
        # 构建用户ID -> 索引映射
        self.userId2index = {uid: i for i, uid in enumerate(self.user_ids)}
    
    def recall(self, user_id, top_k=10, return_scores=True):
        if user_id not in self.userId2index:
            return []  # 用户不存在
        
        u_idx = self.userId2index[user_id]
        u_vec = self.user_emb[u_idx]                  # (emb_dim,)
        
        # 点积相似度
        scores = self.movie_emb @ u_vec               # (num_movies,)
        top_indices = np.argsort(-scores)[:top_k]     # 取前 top_k
        
        results = [(int(self.movie_ids[i]), float(scores[i])) for i in top_indices]
...
def merge_recalls(user_id, keyword_recaller, llm_recaller, ratings_df, 
                 ...
                 two_tower_recaller=None,  # 新增双塔召回器
                 keyword_weight=0.3, llm_weight=0.5, ad_weight=0.2, two_tower_weight=0.4,
                 top_k=15,
    ...
    # 双塔召回
    two_tower_results = _normalize(dict(
        two_tower_recaller.recall(user_id, top_k=10) if two_tower_recaller else {}
    ))
    
    # 融合所有结果
    all_items = set(keyword_results.keys()).union(
        llm_results.keys(),
        ad_results.keys(),
        two_tower_results.keys()
    )
